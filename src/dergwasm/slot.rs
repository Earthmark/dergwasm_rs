...
extern "C" {
    ...
    pub fn slot__get_name(slot: RefId<Slot>) -> Extern<c_char>;
    pub fn slot__set_name(slot: RefId<Slot>, str: *const c_char);

    pub fn slot__get_num_children(slot: RefId<Slot>) -> i32;
    pub fn slot__get_child(slot: RefId<Slot>, index: u32) -> RefId<Slot>;
...
impl Slot {
    ...
    pub fn get_parent(&self) -> Option<Slot> {
        unsafe { slot__get_parent(self.0.clone()) }.filter_invalid()
    }

    pub fn get_active_user(&self) -> Option<User> {
        unsafe { slot__get_active_user(self.0.clone()) }.filter_invalid()
    }

    pub fn get_active_user_root(&self) -> Option<UserRoot> {
        unsafe { slot__get_active_user_root(self.0.clone()) }.filter_invalid()
    }

    pub fn get_object_root(&self, only_explicit: bool) -> Option<Slot> {
        unsafe { slot__get_object_root(self.0.clone(), if only_explicit { 1 } else { 0 }) }
            .filter_invalid()
    }

    pub fn get_name(&self) -> Option<String> {
        let str = unsafe { slot__get_name(self.0.clone()) };
